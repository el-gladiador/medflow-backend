...
CREATE INDEX idx_invitations_token_hash ON user_invitations(token_hash) WHERE status = 'pending';
...
CREATE INDEX idx_invitations_created_by ON user_invitations(created_by);

-- Unique constraint: only one pending invitation per email
CREATE UNIQUE INDEX idx_invitations_unique_pending_email ON user_invitations(email) WHERE status = 'pending';
